const instialState = {
  ...
  const signIn = (state = instialState, action) => {
    ...
    switch (type) {
      ...
      case "LOGIN":
        const { token,userId,userName,avatar } = payload;
        localStorage.setItem("token", token);
        localStorage.setItem("userId", userId);
        localStorage.setItem("userName", userName);
        localStorage.setItem("avatar", avatar);
        return { token,userId,userName,avatar }; // << here return token after the value change contain token for user
  

case "LOGOUT"://<< this type clear token and localstorage 
        localStorage.clear();
        return { token:"",userId:"",userName:"",avatar:"" };
      ...
      default: //<<this default return state if there no any change will back for me initial value for state 
        ...
        else return { token:tokenStorage,userId:userIdStorage,userName:userNameStorage,avatar:avatarStorage};
    }
  };
